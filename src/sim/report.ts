...
export interface PairingReport {
  ...
  cardUse: { id: string; drafted: number; fired: number; rate: number; medianCharges: number }[];
...
export function analyse(records: MatchOutcomeRecord[], levels: [Level, Level]): PairingReport {
  ...
  const bands: Band[] = [
    ...
    },
    {
      ...
      ok: true,
      informational: true,
      detail: `${medCharges} (if this sits at 1-2 the charge system is not doing its job)`,
    },
  ];
...
export function renderMarkdown(reports: PairingReport[]): string {
  ...
  for (const r of reports) {
    ...
    lines.push('');
    lines.push(`Win rate: P0 ${pct(r.winRate[0])} / P1 ${pct(r.winRate[1])} / draw ${pct(r.drawRate)}`);
    lines.push('');
    ...
    lines.push('');
    lines.push('| Card | Drafted | Fired | Rate | Median charges when fired |');
    lines.push('| --- | --- | --- | --- | --- |');
    for (const c of r.cardUse.slice().sort((a, b) => a.rate - b.rate)) {
      lines.push(
        `| ${CARDS[c.id].name} | ${c.drafted} | ${c.fired} | ${pct(c.rate)} | ${c.medianCharges} |`,
      );
    }
    lines.push('');
    ...
    for (const s of r.shipUse.slice().sort((a, b) => a.rate - b.rate)) {
      lines.push(`| ${SHIPS[s.id].name} | ${SHIPS[s.id].type} | ${s.drafted} | ${s.used} | ${pct(s.rate)} |`);
    }
    lines.push('');
  }
  return lines.join('\n');
