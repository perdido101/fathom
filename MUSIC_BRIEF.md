...
| --- | --- | --- | --- |
| Battle | `battle.mp3` | 180s, loops | 2.6 MB |
| Menu | `menu.mp3` | 120s, loops | 1.8 MB |
| Draft | `draft.mp3` | 90s, loops | 1.4 MB |
| Deploy | `deploy.mp3` | 75s, loops | 1.2 MB |
| Bracket | `bracket.mp3` | 110s, loops | 1.7 MB |
| Victory | `victory.mp3` | 25s, one-shot | 0.5 MB |
| Defeat | `defeat.mp3` | 25s, one-shot | 0.5 MB |
| Champion | `champion.mp3` | 35s, one-shot | 0.7 MB |

**Total budget: 10.4 MB** for the full set. At 128 kbps
...
| | |
| --- | --- |
| **File** | `src/ui/music/files/battle.mp3` |
| **Plays on** | The battle screen, for as long as a match lasts |
| **Length** | 180s |
| **Loops?** | Yes — see the loop note above |
| **Ceiling** | 2.6 MB |
...
**Must not fight with:** The 20-second decision clock, the ticking under five seconds, and up to nine impacts landing in one round. This track is the floor those sit on and must never be the loudest thing in the mix.
...
| | |
| --- | --- |
| **File** | `src/ui/music/files/menu.mp3` |
| **Plays on** | Main menu, leaderboard, season, settings, credits |
| **Length** | 120s |
| **Loops?** | Yes — see the loop note above |
| **Ceiling** | 1.8 MB |
...
**Must not fight with:** Nothing much — this is the one screen with no clock. It can be the loudest track in the set.
...
### Draft — choosing in the dark

| | |
| --- | --- |
| **File** | `src/ui/music/files/draft.mp3` |
| **Plays on** | Both drafts |
| **Length** | 90s |
| **Loops?** | Yes — see the loop note above |
| **Ceiling** | 1.4 MB |
...
**Must not fight with:** The five-beat pick sequence — cards dealing in, your pick lifting, their card back sliding in, the collision. Those are the drama; this is the room they happen in.
...
### Deploy — the last private decision

| | |
| --- | --- |
| **File** | `src/ui/music/files/deploy.mp3` |
| **Plays on** | The deployment screen |
| **Length** | 75s |
| **Loops?** | Yes — see the loop note above |
| **Ceiling** | 1.2 MB |
...
**Must not fight with:** Nothing. This is the quietest screen in the game and the track should keep it that way.
...
### Bracket — eight seats, one pot

| | |
| --- | --- |
| **File** | `src/ui/music/files/bracket.mp3` |
| **Plays on** | The tournament bracket and the forming screen |
| **Length** | 110s |
| **Loops?** | Yes — see the loop note above |
| **Ceiling** | 1.7 MB |
...
**Must not fight with:** Chips stacking as seats fill, and the round-win sting between matches. It should thin out rather than compete.
...
### Victory — under the slam

| | |
| --- | --- |
| **File** | `src/ui/music/files/victory.mp3` |
| **Plays on** | Under the victory slam and the result screen after a win |
| **Length** | 25s |
| **Loops?** | No. Plays once and ends. |
| **Ceiling** | 0.5 MB |
...
**Must not fight with:** The victory sound cue, which lands in the same instant. The cue is the punctuation; this is the sentence after it, so the first second should be nearly empty.
...
### Defeat — built as carefully as the win

| | |
| --- | --- |
| **File** | `src/ui/music/files/defeat.mp3` |
| **Plays on** | Under the defeat slam and the result screen after a loss |
| **Length** | 25s |
| **Loops?** | No. Plays once and ends. |
| **Ceiling** | 0.5 MB |
...
**Must not fight with:** The defeat cue, in the same instant. Same rule: leave the first second nearly empty.
...
### Champion — the loudest thing in the game

| | |
| --- | --- |
| **File** | `src/ui/music/files/champion.mp3` |
| **Plays on** | The champion screen |
| **Length** | 35s |
| **Loops?** | No. Plays once and ends. |
| **Ceiling** | 0.7 MB |

The champion screen, after winning a bracket final. Eight entered; this plays for one of them. Not looped.
...
**Must not fight with:** Nothing. This is the one track allowed to be the loudest thing on screen.
